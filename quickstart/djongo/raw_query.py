...
def raw_agg_complex_nested_query():
    ...
    pipeline = [
        ...
        {"$match": {
            "configuration.sources.connection_type.type": "mongodb"
        }},
        {"$group": {
            "_id": {
                "bot_name": "$name",
                "source_category": "$configuration.sources.category"
            },
            "source_count": {"$sum": 1}
        }},
        {"$sort": {"source_count": -1}},
...
def run_raw_aggregation_tests():
    ...
    print("🧪 Running Raw PyMongo Aggregation Pipeline Performance Tests")
    print("=" * 65)
    ...
    # SUMMARY
    print("\n📊 RAW PYMONGO AGGREGATION PERFORMANCE SUMMARY")
    print("=" * 65)
    ...
    print("📁 Results saved for comparison with Djongo ORM aggregations")
